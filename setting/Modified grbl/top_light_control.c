...
void top_light_init()
{
  TOP_LIGHT_DDR |= (1 << TOP_LIGHT_BIT); // Configure as output pin.
  top_light_stop();
}
...
{
    uint8_t lgt_state = TOP_LIGHT_STATE_DISABLE;
	
	if ((TOP_LIGHT_PORT |= (1 << TOP_LIGHT_BIT))) {
    	lgt_state |= TOP_LIGHT_STATE;
    return (lgt_state);
  	}
}
...
void top_light_stop()
{
  TOP_LIGHT_PORT &= ~(1 << TOP_LIGHT_BIT);
}
...
void top_light_set_state(uint8_t mode)
{
  ...
  if (mode == TOP_LIGHT_DISABLE) {
    top_light_stop();
  }
  else {
    if ( mode == TOP_LIGHT_ENABLE) {
      TOP_LIGHT_PORT |= (1 << TOP_LIGHT_BIT);
    }
  }
