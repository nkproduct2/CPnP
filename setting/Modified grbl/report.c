...
// Prints Vacuum value.
void report_vacuum_message(uint16_t vacuum_val)
{
  printPgmString(PSTR("vacuum:"));
	if (vacuum_val <= 200) vacuum_val = 200;
	if (vacuum_val >= 710) vacuum_val = 710;
	vacuum_val = (vacuum_val-200)/2;
  print_uint8_base10(vacuum_val);
  report_util_line_feed();
}
